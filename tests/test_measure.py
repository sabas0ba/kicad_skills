...
def rc_lowpass_ac(fc=1000.0, points=2001):
    ...
    return make_plot("AC Analysis", "frequency", freq, {"v(out)": response},
                     flags=("complex",))
...
def test_ac_unity_gain_and_phase_margin():
    ...
    result = measure.measure(make_plot("AC Analysis", "frequency", freq,
                                       {"v(out)": response}, flags=("complex",)))
    out = result["signals"]["v(out)"]
...
def test_dc_measurements():
    ...
    out = measure.measure(make_plot("DC transfer characteristic", "v-sweep", sweep,
                                    {"v(out)": 2 * sweep}))
    signal = out["signals"]["v(out)"]
...
def test_thd_of_a_clipped_sine():
    ...
    clean = make_plot("Transient Analysis", "time", t, {"v(out)": np.sin(2 * np.pi * fundamental * t)})
    result = measure.thd(clean, "v(out)", fundamental)
    assert result["thd_percent"] < 0.5

    # a square wave has a well known THD of ~48 %
    square = make_plot("Transient Analysis", "time", t,
                       {"v(out)": np.sign(np.sin(2 * np.pi * fundamental * t))})
