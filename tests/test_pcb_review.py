...
def board_from(footprints=(), tracks=(), vias=(), zones=(), edges=None, silk=()):
    ...
    board.layers = [{"id": "0", "name": "F.Cu", "type": "signal", "user_name": ""},
    ...
    board.edges = edges if edges is not None else [
        {"type": "gr_rect", "points": [(0, 0), (50, 40)]}
    ]
...
def pad(number, x, y, net, size=(1.0, 1.0), type_="smd", drill=None):
    return pcb.Pad(number=number, type=type_, shape="rect", x=x, y=y, angle=0,
                   size=size, drill=drill, layers=["F.Cu"], net=net)


def footprint(ref, x, y, pads, layer="F.Cu", attrs=("smd",)):
    return pcb.Footprint(ref=ref, value="", lib_id="lib:fp", x=x, y=y, angle=0,
                         layer=layer, pads=list(pads), attributes=list(attrs))


def track(x1, y1, x2, y2, width=0.25, net="", layer="F.Cu"):
    return pcb.Track(start=(x1, y1), end=(x2, y2), width=width, layer=layer,
                     net_code=1, net=net)
...
def test_small_vias_and_annular_rings():
    vias = [pcb.Via(x=5, y=5, size=0.45, drill=0.25, layers=["F.Cu", "B.Cu"],
                    net_code=1, net="GND")]
...
def test_decoupling_distance():
    ic = footprint("U1", 10, 10, [pad(str(i), 10 + i, 10, "SIG") for i in range(1, 7)]
                   + [pad("7", 10, 10, "+3V3")])
...
def test_drill_variety():
    vias = [pcb.Via(x=0, y=0, size=0.8, drill=0.3 + i * 0.05, layers=[], net_code=1, net="A")
            for i in range(8)]
    assert "fab.many_drill_sizes" in rules_of(pcb_review.rule_drill_variety(ctx_for(board_from(vias=vias))))
...
def test_drc_violations_become_findings():
    drc = {
        ...
        "unconnected_items": [{"type": "unconnected_items", "severity": "warning",
