...
class PcbContext:
    def __init__(self, target: str | os.PathLike[str], *, use_cli: bool = True,
    ...
    def ic_footprints(self) -> list[pcb.Footprint]:
        return [fp for fp in self.board.footprints
...
def rule_drc(ctx: PcbContext) -> list[Finding]:
    ...
    if not ctx.drc:
        return [Finding("drc.unavailable", "info",
    ...
    for key, prefix in buckets:
        for violation in ctx.drc.get(key, []) or []:
            ...
            where = "; ".join(str(it.get("description", "")) for it in items if it.get("description"))
            findings.append(
                Finding(
...
def rule_outline(ctx: PcbContext) -> list[Finding]:
    ...
    if bbox is None:
        return [Finding("board.no_outline", "error",
                        "no Edge.Cuts geometry: the board has no outline")]
    ...
    if size[0] < 5 or size[1] < 5:
        findings.append(Finding("board.tiny_outline", "warning",
...
def rule_vias(ctx: PcbContext) -> list[Finding]:
    ...
    if small:
        findings.append(
            Finding("via.small_drill", "warning",
    ...
    if tight:
        findings.append(
            Finding("via.annular_ring", "warning",
...
def rule_drill_variety(ctx: PcbContext) -> list[Finding]:
    ...
    if len(drills) > limit:
        return [Finding("fab.many_drill_sizes", "info",
...
def rule_unrouted(ctx: PcbContext) -> list[Finding]:
    ...
    for net, pads in sorted(ctx.pads_by_net.items()):
        ...
        if net in routed_nets or net in zone_nets:
            continue
        findings.append(
            Finding("route.unrouted_net", "error",
...
def rule_edge_clearance(ctx: PcbContext) -> list[Finding]:
    ...
    for fp in ctx.board.footprints:
        for pad in fp.pads:
            margin = min(pad.x - min_x, max_x - pad.x, pad.y - min_y, max_y - pad.y) - max(pad.size) / 2
            if margin < clearance:
                offenders.append(f"{fp.ref}.{pad.number}")
    if offenders:
        return [Finding("board.edge_clearance", "warning",
...
def rule_decoupling_placement(ctx: PcbContext) -> list[Finding]:
    ...
    for fp in ctx.ic_footprints():
        supply_pads = [p for p in fp.pads
                       if p.net and ctx.net_class_of(p.net) == "power"]
        for pad in supply_pads:
            ...
            if not caps:
                findings.append(
                    Finding("layout.no_decoupling", "warning",
                            f"{fp.ref}.{pad.number} ({pad.net}) has no capacitor on that net",
                            location=f"{fp.ref}.{pad.number}")
                )
                continue
            distance = min(math.dist((pad.x, pad.y), (cap_pad.x, cap_pad.y))
                           for _, cap_pad in caps)
            if distance > limit:
                nearest = min(caps, key=lambda c: math.dist((pad.x, pad.y), (c[1].x, c[1].y)))[0]
                findings.append(
                    Finding("layout.decoupling_distance", "warning",
...
def rule_ground_plane(ctx: PcbContext) -> list[Finding]:
    ...
    ground_zones = [z for z in ctx.board.zones
                    if ctx.net_class_of(z.net) == "ground" and not z.keepout]
    if not ground_zones:
        return [Finding("layout.no_ground_plane", "warning",
    ...
    if unfilled:
        return [Finding("layout.unfilled_zone", "warning",
...
def rule_silkscreen(ctx: PcbContext) -> list[Finding]:
    ...
    missing = [fp.ref for fp in ctx.board.footprints
               if fp.ref and fp.ref not in labelled and "virtual" not in fp.attributes]
    if missing:
        findings.append(
            Finding("silk.missing_reference", "info",
...
def rule_placement(ctx: PcbContext) -> list[Finding]:
    ...
    if bbox:
        min_x, min_y, max_x, max_y = bbox
        outside = [fp.ref for fp in ctx.board.footprints
                   if not (min_x <= fp.x <= max_x and min_y <= fp.y <= max_y)]
        if outside:
            findings.append(
                Finding("layout.outside_outline", "error",
    ...
    if bottom:
        findings.append(
            Finding("layout.double_sided_assembly", "info",
...
def rule_mounting_and_testpoints(ctx: PcbContext) -> list[Finding]:
    ...
    if not any(r.startswith(("H", "MH")) for r in refs):
        findings.append(Finding("mechanical.no_mounting_holes", "info",
                                "no mounting hole footprint (H*/MH*) found"))
    if not any(r.startswith("TP") for r in refs):
        findings.append(Finding("test.no_testpoints", "info",
                                "no test point footprints (TP*) found - "
                                "consider adding them for bring-up"))
    return findings
...
def rule_layer_usage(ctx: PcbContext) -> list[Finding]:
    ...
    return [Finding("route.layer_usage", "info",
        ...
        (
            {
                ...
                "track_length_mm": round(
                    sum(t.length for t in board.tracks if t.net == name), 2
                ),
