...
| `route.hairpin` | 100 deg / 1.2 mm | a run that turns back on itself over two adjacent corners: a 90 and a 45 with a tenth of a millimetre between them passes the angle rule corner by corner and still reads as one folded bend. Signed turns, so a staircase's alternating 45s cancel; a fold whose middle sits inside its own pad is the escape fan's deliberate micro-hook and stays |
