...
| `silk.under_part` | — | a silkscreen string inside another footprint's courtyard on the same side: it prints on the bare board and is hidden the moment that part is fitted. A string's own footprint is not its neighbour — a designator sits inside its own courtyard by convention. Fires 11 times across KiCad's 18 demo boards, which is why it is a warning: a string a fraction inside a neighbour's courtyard is a human's call, and on a generated board the fix is to move it |
