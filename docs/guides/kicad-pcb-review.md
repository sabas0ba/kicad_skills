...
| `silk.under_part` | — | a silkscreen string inside another footprint's courtyard on the same side: it prints on the bare board and is hidden the moment that part is fitted. A string's own footprint is not its neighbour — a designator sits inside its own courtyard by convention |
