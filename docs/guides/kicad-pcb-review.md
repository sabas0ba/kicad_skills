...
| `layout.solid_pad_connection` | — | a filled zone that floods its own *through-hole* pads with solid copper instead of relieving them thermally. A plane is a heat sink: an iron pours its heat into a hundred square millimetres of copper and the joint never wets. Surface pads are not counted — they reflow with the board — and thermal relief is KiCad's own default, so a zone set to solid usually means somebody turned it off without meaning to |
