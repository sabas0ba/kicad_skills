...
* **Never run `kicad-cli`, `ngspice` or the `eda` CLI on the host.** Use
  `./bin/eda <command>` — it builds and runs the container, maps your uid, and
  keeps the host clean. Anything else pollutes the host or fails outright.
* Paths passed to `./bin/eda` are relative to the repository root (mounted at
  `/work` in the container).
* `KICAD_VERSION` selects the KiCad release; the default is in `bin/eda` and
